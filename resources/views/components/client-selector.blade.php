    ...
    <form method="POST" action="{{ route('client.switch') }}" @submit="submitting = true">
        ...
        <div class="relative">
            <select
                ...
                class="block w-full appearance-none pr-10 border border-gray-300/20 bg-gray-300/10 rounded-md px-3 py-2 text-xs text-gray-300 focus:outline-none focus:ring-none transition-colors disabled:bg-gray-100 disabled:text-gray-300 disabled:cursor-not-allowed"
                :disabled="submitting"
            >
                @if (count($clients) === 0)
            ...
            <div class="pointer-events-none absolute inset-y-0 right-3 flex items-center">
                <x-lucide-chevron-down class="size-3 text-gray-400" />
            </div>
        </div>
    </form>
</div>
